...
export default async function ContactDetailPage(props: PageProps<"/contacts/[id]">) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      {contact.firm && <p className="text-sm text-neutral-500">{contact.firm}</p>}

      <div className="mt-3 flex flex-wrap gap-2 text-xs">
        <span className="rounded bg-neutral-100 px-1.5 py-0.5">{contact.type.replace("_", " ").toLowerCase()}</span>
        <span className="rounded bg-neutral-100 px-1.5 py-0.5">warmth: {contact.warmthTier.toLowerCase()}</span>
        <span className="rounded bg-neutral-100 px-1.5 py-0.5">
          cadence: every {cadence}d
        </span>
        {overdue ? (
          ...
          <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-emerald-800">on cadence</span>
        )}
      </div>

      {contact.notes && (
        <div className="mt-4 rounded-lg border border-neutral-200 bg-white p-3 text-sm text-neutral-700">
          {contact.notes}
        </div>
      )}

      <h2 className="mt-6 text-sm font-medium text-neutral-700">Linked opportunities</h2>
        ...
        {contact.opportunities.map((o) => (
          <li key={o.id}>
            ...
            >
              ...
              <span className="ml-2 text-xs text-neutral-400">{o.stage.toLowerCase()}</span>
            </Link>
          </li>
        ))}
        {contact.opportunities.length === 0 && (
          <p className="text-sm text-neutral-400">No linked opportunities.</p>
        )}
      </ul>
